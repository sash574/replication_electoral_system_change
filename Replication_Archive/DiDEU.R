...
res$m2b <- est_pdid(f, dat, ~1 + RAIIP  + ch_imp_fns + opencIP + 
   S_uegenIP, pDmethod="blogit")

res$m2c <- est_pdid(f, dat[dat$ep_election>1979 & dat$ccode!=6,], ~1 + RAIIP + ch_imp_fns + 
   opencIP + S_uegenIP + migr_inflIP, pDmethod="blogit")

save(res, file="DiDEU_est.Rdata")
