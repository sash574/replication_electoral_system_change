...
# merge with explicit match indicator
mergeDS <- function(x,y, by = intersect(names(x), names(y))){
   x[is.na(x)] <- Inf
   y[is.na(y)] <- Inf
...
gzread <- function(name){
   dat <- as.data.table(fread(cmd = paste0("gunzip -c ", name)))
   return(dat)
}

# Read DTA files, return DT obj
sread <- function(name){
   dat <- as.data.table(read.dta13(name))
   return(dat)
}
...
# Func: z-std variable (option gelman == div. by 2SD)
...
###############################################################################
# end: Misc functions [DS]
###############################################################################



###############################################################################
# Model est and summary functions [DS]
###############################################################################

# Func: estimates simple DiD model via OLS
# extracts 'ATT'. Heterosc.robust SE and Wild b/strap SE
est_did <- function(data, formula){
   dat <- data
   out <- lm(formula, data=dat)
   est <- coef(out)["post:D"]
   se <- sqrt(diag(vcovHC(out, type="HC1")))["post:D"]
   se_b <- sqrt(diag(vcovBS(out, R=5000L, type="wild-rademacher")))["post:D"]
   ret <- rbind(est,se, se_b)
   colnames(ret) <- c("ATT")
   rownames(ret) <- c("est", "se", "se_b")
   return(ret)
}


# Func: estimates multiperiod DiD model
# extracts ATT, ATT_e, CvM test from output
# Arg: formula for x (unconditional or conditional parallel trends)
est_pdid <- function(f, data, xformula=~1, pDmethod="logit"){
   dat <- data
   out <- mp.spatt(f, data=dat, tname="year", panel=T, idname="ccode", first.treat.name="fD", bstrap=T, biters=5000L, clustervars="ccode", xformla = xformula, method=pDmethod)
   out$cvm <- mp.spatt.test(f, xformlalist=list(xformula), data=dat, panel=T, first.treat.name="fD", idname="ccode", tname="year", clustervarlist=list("ccode"), method=pDmethod) 

   att <- rbind(out$aggte$simple.att, out$aggte$simple.se)
   att.e <- rbind(out$aggte$dynamic.att.e, out$aggte$dynamic.se.e)
   CvM <- rbind(out$cvm[[1]]$CvM, out$cvm[[1]]$CvMpval)
   W <- rbind(out$W, out$Wpval)
   ret <- cbind(att, att.e, CvM, W)
   colnames(ret) <- c("ATT", paste0("ATT_t",1:4), "CvM", "W")
   rownames(ret) <- c("est", "se/p")
   return(ret)
}


###################################################################################
# end: Model est and summary functions [DS]
###################################################################################
...
###################################################################################
# Multi-period DiD [Callaway and Sant'Anna 2018]
###################################################################################

# Source: Callaway, Brantly and Sant'Anna, Pedro.  "Difference-in-Differences with Multiple 
#  Time Periods and an Application on the Minimum Wage and Employment." Working Paper 
#  <https://ssrn.com/abstract=3148250> (2018). 
# Edited, DS, Sept 30 2019. Added method "blogit" option.
# Edited, DS, Oct 2 2019. Added propensity score summaries
#
...
#'  are included. Options are logit (the default) and blogit which uses 
#'  Gelman et al 2008 (Annals of Appl Stats) cauchy priors
    ...
    for (f in 1:flen) {
        ...
        for (t in 1:(tlen-1)) {
            ...
            if (panel) {
                ...
                if(method=="blogit"){
                    pscore.reg <- arm::bayesglm(pformla, family=binomial(link="logit"),
                                  data=subset(disdat, C+G==1),
                                  prior.scale.for.intercept = 5,
                                  maxit = 500, scaled = TRUE)
                }
                ...
                cat("Panel P(D) min/max", sprintf("%.2f", c(min(pscore), max(pscore))), "\n")
...
###################################################################################
# end: Multi-period DiD [Callaway and Sant'Anna 2018]
###################################################################################
